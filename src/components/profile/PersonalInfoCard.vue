<!-- src/components/profile/PersonalInfoCard.vue -->
<template>
  <div>
        ...
        <div class="flex-1">
          ...
          <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
            ...
            <div v-if="person?.phone">
              ...
              <p class="text-sm font-medium text-gray-800 dark:text-white/90">
                {{ formatPhone(person.phone, person.indicatif) }}
              </p>
            </div>

            <div v-if="person?.location">
              <p class="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">
                Localisation
              </p>
              <p class="text-sm font-medium text-gray-800 dark:text-white/90">
    ...
    <!-- Modal d'édition -->
    <Modal v-if="isProfileInfoModal" @close="closeEditModal">
      <template #body>
        <div
          ...
          <form @submit.prevent="saveProfile" class="flex flex-col">
            <div class="custom-scrollbar h-[440px] overflow-y-auto p-2">
              <div class="mt-2">
            ...
            <div class="flex items-center gap-3 px-2 mt-6 lg:justify-end">
              <button @click.prevent="closeEditModal" type="button"
...
// ✅ Computed pour person
const person = computed(() => userPerson.value)

// ✅ Formater le téléphone
const formatPhone = (phone, indicatif) => {
  if (!phone) return 'N/A'
  return indicatif ? `${indicatif} ${phone}` : phone
}
...
const saveProfile = async () => {
  ...
  try {
    ...
    console.log('Profil mis à jour:', formData.value)
