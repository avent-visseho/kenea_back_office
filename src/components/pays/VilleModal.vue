            ...
            <div v-if="isOpen" class="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-900"
              ...
              <form @submit.prevent="handleSubmit" class="space-y-4">
                <div>
                  ...
                  <input v-model="formData.name" type="text" required placeholder="Ex: Paris"
                    class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 dark:border-gray-700 dark:bg-gray-800 dark:text-white dark:placeholder:text-gray-500" />
                </div>
                ...
                <div>
                  ...
                  <select v-model="formData.paysId" required
                    class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 dark:border-gray-700 dark:bg-gray-800 dark:text-white">
                ...
                <div class="flex gap-3 pt-4">
                  ...
                  <button type="submit" :disabled="isSubmitting"
                    class="flex-1 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50">
...
const { createCities, updateCities, paysList, fetchPaysList } = usePaysVille()
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEditMode.value) {
      result = await updateCities(props.villeData.id, formData.value)
    } else {
      result = await createCities(formData.value)
    }
