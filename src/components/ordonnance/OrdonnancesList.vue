<!-- src/components/ordonnances/OrdonnancesList.vue -->
<template>
          ...
          >
            ...
            <td class="py-3 whitespace-nowrap">
              <div class="flex items-center gap-2">
                <button
                  @click="viewOrdonnance(ordonnance)"
                ...
                <button
                  v-if="ordonnance.etat === 'PENDING'"
                  @click="changeStatus(ordonnance, 'VALIDATED')"
                ...
                <button
                  v-if="ordonnance.etat === 'PENDING'"
                  @click="changeStatus(ordonnance, 'REJECTED')"
                ...
                <button
                  @click="confirmDelete(ordonnance)"
...
const filteredOrdonnances = computed(() => {
  ...
  if (selectedStatus.value) {
    filtered = filtered.filter(o => o.etat === selectedStatus.value)
  }

  if (searchQuery.value) {
    const query = searchQuery.value.toLowerCase()
    filtered = filtered.filter(o => o.code?.toLowerCase().includes(query) || o.id?.toLowerCase().includes(query))
  }
...
const changeStatus = async (ordonnance, newStatus) => {
  if (confirm(`Êtes-vous sûr de vouloir ${newStatus === 'VALIDATED' ? 'valider' : 'rejeter'} cette ordonnance ?`)) {
