      ...
      <div
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                Traiter une progression
              </h3>
              <p class="text-sm text-gray-500 dark:text-gray-400">
                Ordonnance ID: {{ progression.id }}
              </p>
            </div>
          </div>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="flex gap-3">
              ...
              <div class="text-sm text-blue-800 dark:text-blue-300">
                ...
                <ul class="list-disc list-inside space-y-0.5 text-xs">
                  <li>Ajoutez les articles à traiter</li>
                  <li>Vérifiez la disponibilité et les prix</li>
                  <li>Validez le traitement</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div v-if="items.length === 0" class="rounded-lg border border-dashed border-gray-300 p-6 text-center dark:border-gray-600">
              <p class="text-sm text-gray-500 dark:text-gray-400">Aucun article ajouté</p>
            </div>

            <div v-else class="space-y-3">
              <div v-for="(item, index) in items" :key="index"
                class="rounded-lg border border-gray-200 p-4 dark:border-gray-700 dark:bg-gray-800/50">
                <div class="flex justify-between items-start mb-3">
                  <h4 class="font-medium text-gray-900 dark:text-white">Article {{ index + 1 }}</h4>
                  <button type="button" @click="removeItem(index)"
                    class="p-1 text-red-600 hover:bg-red-50 rounded-lg dark:text-red-400 dark:hover:bg-red-900/20">
                ...
                <div class="grid grid-cols-2 gap-3 mb-3">
                  <div>
                    ...
                    <input v-model="item.nomProduit" type="text" placeholder="Ex: Amoxicilline"
                  ...
                  <div>
                    ...
                    <input v-model.number="item.quantite" type="number" min="1" placeholder="0"
                      class="w-full px-3 py-2 rounded-lg border border-gray-300 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3 mb-3">
                  <div>
                    ...
                    <input v-model.number="item.prixUnitaire" type="number" min="0" step="0.01" placeholder="0.00"
                  ...
                  <div>
                    ...
                    <div class="px-3 py-2 rounded-lg border border-gray-300 bg-gray-50 text-sm font-medium text-gray-900 dark:border-gray-600 dark:bg-gray-900 dark:text-white">
          ...
          <!-- Total Section -->
          <div v-if="items.length > 0" class="rounded-lg bg-gradient-to-r from-blue-50 to-blue-100 p-4 dark:from-blue-900/20 dark:to-blue-900/10 border border-blue-200 dark:border-blue-900/30">
            <div class="flex justify-between items-center">
              <span class="text-lg font-semibold text-gray-900 dark:text-white">Total:</span>
              <span class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ totalAmount.toFixed(2) }}€</span>
            </div>
          </div>
          ...
          <div class="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-800">
            ...
            <button type="submit" :disabled="isLoading || items.length === 0"
              ...
              {{ isLoading ? 'Traitement...' : 'Valider le traitement' }}
            </button>
          </div>
        </form>
      </div>
...
const handleSubmit = async () => {
  ...
  if (result.success) {
    ...
    error.value = result.error || 'Erreur lors du traitement'
  }
}

onMounted(() => {
  // Récupérer l'ID utilisateur depuis le stockage ou autre source
  // Pour l'instant, utiliser une valeur par défaut
  userId.value = sessionStorage.getItem('userId') || 'current-user'
  addItem()
})
</script>